...
Live JSON-LD is this system. Older `layouts/partials/seo/structured/*.html` and `social/*.html` look unused by `head.html`. Cascade keys use Schema.org camelCase (`sameAs`). DOI is `identifier` as well as `sameAs`. Do not put page-level flags like `notnews` inside the `jsonld:` map.
