...
1. **JSON-LD hygiene** (templates): cascade `sameAs`, DOI `identifier`, Organization `@id`, author `notnews`, `ImageObject`, jsonld string escaping, empty `twitter:creator` — implemented. Remaining: SearchAction/InstantSearch routing with goal 3.
